...
def parse_arguments():
    ...
    parser.add_argument(
        '--mouse',
        action='store_true',
        help='If set, use mouse database, otherwise use human.'
    )
    ...
    if len(r1_fastq) == 1 and len(r2_fastq) == 1:
        try:
            ...
            if success:
                ...
                path_to_bcr = os.path.join(subdir, f"{basename}_merged.BCR.fa")
                path_to_unitigs = os.path.join(subdir, f"{basename}_merged.unitigs.fa")
