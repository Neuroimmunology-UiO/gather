...
def find_matching_indices(array1, array2, array2_rc, weights, num_jobs):
    ...
    def compare_arrays(array1, array2, num_jobs):
        ...
        parallel_results = Parallel(n_jobs=num_jobs)(
            delayed(compare_chunk)(start, end) for start, end in tqdm(chunks, desc='Processing unitigs')
        )
...
def find_optimum_path(G, wcc, unitigs, weights):
    ...
    for cluster in tqdm(wcc, desc="Finding the optimum path in WCCs"):
...
def local_alignment_best(seq1, seq2, match_score=3, mismatch_penalty=-3, gap_open_penalty=-5, gap_extend_penalty=-1):
    """
...
def find_best_alignments(seqs1, seqs2, num_jobs):
    ...
    results = Parallel(n_jobs=num_jobs)(
        ...
        for i, j in tqdm(all_combinations, total=total_combinations, desc='Processing all combinations')
    )
...
def process_alignment(seqs, headers, seqs_contigs, header_c, output_bcr_path, num_jobs):
    ...
    filtered_df = df[(df['SCORE'] >= 100.0) & (df['ALIGN_FRACTION'] >= 0.95)]
...
def classify_reads(headers, sequences):
    ...
    classified_reads = {
        'BCR_heavy_chain': {chain: [] for chain in bcr_heavy_chain},
        'BCR_light_chain': {chain: [] for chain in bcr_light_chain}
    }
...
def parse_arguments():
    ...
    parser.add_argument('--db', default='/home/mojtaba/Sys_admin/G-BAT/db/human/BCR',  help='Path to the database directory')
...
def main():
    ...
    input_unitigs_name = os.path.basename(input_rna_fastq).replace('.fastq.gz', '.fastq.unitigs.fa')
    ...
    Contigs = [
        ...
        for path in tqdm(optimum_paths, desc='Making contigs from optimum paths')
    ]
    ...
    output_contigs_name = input_unitigs_name.replace('unitigs', 'BCR')
    ...
    if args.merge_bcrs:
        ...
        output_highest_expressed_BCR = input_unitigs_name.replace('unitigs', 'BCR_HE')
