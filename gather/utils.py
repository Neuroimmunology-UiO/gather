...
class CommandExecutor:
    ...
    def run_concat_fastqs(self, r1_file, r2_file=None, output_dir="output"):
        ...
        If only R1 is provided, it will be used alone.

        Output filename is derived from the R1 filename:
          <prefix>R1*.fastq.gz -> <prefix>_merged.fastq.gz
          <prefix>r1*.fq.gz    -> <prefix>_merged.fq.gz
        """
        os.makedirs(output_dir, exist_ok=True)

        # Derive output name by replacing R1/r1 with merged
        basename = os.path.basename(r1_file)
        output_file = re.sub(
            r"[._-]?[Rr]1[^/]*\.(fastq|fq)\.gz$",
            r"_merged.\1.gz",
            basename,
        )

        if output_file == basename:
            raise ValueError("R1 filename must contain 'R1' or 'r1' before .fastq.gz/.fq.gz")

        output_file = os.path.join(output_dir, output_file)

        # Build input list (ignore None)
        inputs = [r1_file] + ([r2_file] if r2_file else [])

        # Concatenate
        with open(output_file, "wb") as out_f:
            result = subprocess.run(["cat"] + inputs, stdout=out_f)

        if result.returncode != 0:
            raise RuntimeError(f"Concatenation failed with return code {result.returncode}")
