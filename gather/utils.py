...
class CommandExecutor:
    ...
    def run_concat_fastqs(self, r1_file, r2_file=None, output_dir="output"):
        """
        Concatenate R1 and R2 FASTQ(.gz) files into a single merged FASTQ(.gz).
        If only one file is provided, that file will be used alone.

        Output filename rules:
          - If the filename contains R1/r1 or R2/r2, use everything before that token as the prefix.
          - Otherwise, use everything before the .fastq/.fq[.gz] extension as the prefix.
          - The output keeps the original extension (.fastq or .fq) and .gz if present.
          - Final name: <prefix>_merged.<ext>[.gz]
        """
        os.makedirs(output_dir, exist_ok=True)

        # Use r1_file's naming to define the output name (even if it's actually an R2 file).
        basename = os.path.basename(r1_file)

        # 1) Try to capture prefix before R1/R2 (case-insensitive), then ext + optional .gz
        m = re.match(
            r'^(?P<prefix>.*?)[._-]?[Rr][12][^/]*?\.(?P<ext>fastq|fq)(?P<gz>\.gz)?$',
            basename
        )

        if not m:
            # 2) Fall back to "anything before .fastq/.fq[.gz]"
            m = re.match(
                r'^(?P<prefix>.+?)\.(?P<ext>fastq|fq)(?P<gz>\.gz)?$',
                basename
            )

        if not m:
            raise ValueError(
                "Input filename must end with .fastq(.gz) or .fq(.gz). "
                f"Got: {basename}"
            )
        ...
        output_file = os.path.join(output_dir, f"{prefix}_merged.{ext}{gz}")

        # Build input list (ignore None)
        inputs = [p for p in [r1_file, r2_file] if p]

        if not inputs:
            raise ValueError("At least one FASTQ file must be provided.")

        # Concatenate safely in Python (works for both plain and gz files since we're doing raw byte copy).
    ...
    mouse=False
        ...
        if use_spades:
            command.extend(["--sensitive", "--spades_path", spades_path])
        
        if mouse:
            command.extend(["--mouse"])
...
class ProcessSeq:
    ...
    @staticmethod
    def process_alignment_blast(seqs_contigs, output_bcr_path, blast_output_file, blast_dir,
        ...
        records = []  
        def add_algo(pairs, Ws, algo_name):
            """
            pairs: list of (label, seq)
            Ws:    list of raw weights for this algo
            algo_name: e.g. 'algo_1' or 'algo_2'
            """
            # group maxima (IGH vs non-IGH) for this algorithm
            gmax = {True: 0.0, False: 0.0}
            for (label, _), w in zip(pairs, Ws):
                key = ('IGH' in label)
                if w > gmax[key]:
                    gmax[key] = w

            # append normalized records
            for (label, seq), w in zip(pairs, Ws):
                is_heavy = ('IGH' in label)
                denom = gmax[is_heavy] if gmax[is_heavy] else 1.0
                nw = w / denom
                records.append({
                    'label': label,
                    'seq': seq,
        ...
        add_algo(output_data, Ws, 'algo_2')
        
        heavy = sorted((r for r in records if r['is_heavy']),
                       key=lambda r: r['norm_w'], reverse=True)
        light = sorted((r for r in records if not r['is_heavy']),
                       key=lambda r: r['norm_w'], reverse=True)

        heavy_headers = [f"{r['label']} | {r['algo']} | W: {r['norm_w']:.6f}" for r in heavy]
        heavy_seqs    = [r['seq'] for r in heavy]

        light_headers = [f"{r['label']} | {r['algo']} | W: {r['norm_w']:.6f}" for r in light]
        light_seqs    = [r['seq'] for r in light]
        
        headers = heavy_headers + light_headers
        seqs    = heavy_seqs    + light_seqs
        # Save the sequences in the sorted order
        IO.save_to_fasta(headers, seqs, output_bcr_path, include_header=True, append=False)
    ...
    @staticmethod
    def process_bcr_file(file1=None, read_fasta=None, file2=None):
        """
        ...
        Deduplication rules:
          ...
          - Between files (if both provided): compare families again, keep the longest sequence.
          - Final output: split into heavies and lights, sort each by descending weight,
            heavies always listed first.
        """
        if not callable(read_fasta):
            raise ValueError("You must pass a callable read_fasta function.")
        ...
        # --- read and process files (each may be empty) ---
        file1_entries = process_file(file1)
        file2_entries = process_file(file2)

        if not file1_entries and not file2_entries:
            # choose: return empty or raise
            return [], []

        if file1_entries and file2_entries:
            # compare across files → keep longest sequence per family
            combined = {}
            ...
            # only one side had entries
            all_entries = file1_entries or file2_entries
    ...
    @staticmethod
    def collect_chains(main_dir, output_dir, mouse=False,
                       heavy_name="collected_heavy_chains.fasta",
                       light_name="collected_light_chains.fasta"):
        """
        Collect heavy and light chain sequences only from BCR algo1 and algo2 fasta files.

        Heavy chains -> <output_dir>/collected_heavy_chains.fasta
        Light chains -> <output_dir>/collected_light_chains.fasta

        :param main_dir: Path to directory containing <cell_name>_merged.BCR_algo1/2.fa files
        :param output_dir: Directory where collected fasta files will be saved
        :param heavy_name: Filename for heavy chains fasta
        :param light_name: Filename for light chains fasta
        """
        

        os.makedirs(output_dir, exist_ok=True)

        heavy_file = os.path.join(output_dir, heavy_name)
        light_file = os.path.join(output_dir, light_name)

        # Clear old outputs if exist
        for f in (heavy_file, light_file):
            if os.path.exists(f):
                os.remove(f)

        for fname in os.listdir(main_dir):
            # Process only algo1 and algo2 fasta files
            if not fname.endswith("BCR.fa"):
                continue

            # Extract cell name = anything before _merged
            match = re.match(r"(.+?)_merged\.BCR.fa", fname)
            if not match:
                continue
            cell_name = match.group(1)

            file_path = os.path.join(main_dir, fname)
            headers, sequences = IO.read_fasta(file_path)

            for header, seq in zip(headers, sequences):
                # Join all header parts into one string, stripping trailing commas
                header_str = "_".join([h.rstrip(",") for h in header])

                # Heavy chains
                if header[0].startswith('IGH'):
                    ig_name = f"{cell_name}|{header_str}"
                    IO.save_to_fasta([ig_name], [seq], heavy_file,
                                     include_header=True, append=True)

                # Light chains
                else:
                    ig_name = f"{cell_name}|{header_str}"
                    IO.save_to_fasta([ig_name], [seq], light_file,
                                     include_header=True, append=True)

        return heavy_file, light_file
    
    @staticmethod
    def select_best_bcr(df: pd.DataFrame, merge_light_loci: bool = True) -> pd.DataFrame:
        """
        From a BCR dataframe, select the best sequence per (cell_id, locus) group.

        Steps:
          1. Extract `cell_id` from sequence_id (string before '|').
          2. Extract `weight` from sequence_id (float after 'W:_').
          3. Keep only rows where:
             - productive == 'T'
             - v_call, j_call, c_call, cdr3 are not null
          4. Group by:
             - If merge_light_loci=True: (cell_id, merged_locus) where IGK/IGL are merged
             - Else: (cell_id, locus)
          5. For each group:
             - Choose the row with the highest weight
             - If another row has the same v_call and c_call but a longer sequence,
               keep that one instead.

        Returns:
            A filtered DataFrame with one row per group.
        """

        df = df.copy()

        # 1) Extract cell_id
        df["cell_id"] = df["sequence_id"].str.split("|").str[0]

        # 2) Extract weight (float after "W:_")
        df["weight"] = (
            df["sequence_id"]
            .str.extract(r"W:_([\d\.eE+-]+)")[0]
            .astype(float)
        )

        # Optional merged locus for light chains
        if merge_light_loci:
            df["group_locus"] = df["locus"].replace({"IGK": "IGLK", "IGL": "IGLK"})
        else:
            df["group_locus"] = df["locus"]
        ...
        # Add sequence length for the later check
        filtered["seq_length"] = filtered["sequence"].str.len()

        # 5) Selection logic per group
        def pick_best(group):
            # Highest weight
            top = group.loc[group["weight"].idxmax()]

            # If another row has same v_call+c_call but longer sequence -> prefer that one
            same_calls = group[(group["v_call"] == top["v_call"]) &
                               (group["c_call"] == top["c_call"])]

            if not same_calls.empty:
                longer = same_calls.loc[same_calls["seq_length"].idxmax()]
                if longer["seq_length"] > top["seq_length"]:
                    return longer

            return top

        group_cols = ["cell_id", "group_locus"]
        work_cols = [c for c in filtered.columns if c not in group_cols]

        best_rows = (
            filtered
            .groupby(group_cols, group_keys=True)[work_cols]   # exclude grouping cols
            .apply(pick_best)                                   # your pick_best stays the same
            .reset_index(level=group_cols)                      # bring keys back as columns
    ...
    @staticmethod
    def harmonize_pass_with_best(pass_path: str, best_path: str, out_path: str = None):
        """
        For rows with the same sequence_id, make all shared columns in pass identical
        to those in best. Writes result back to out_path (defaults to overwriting pass).
        """
        if out_path is None:
            out_path = pass_path

        df_pass = pd.read_csv(pass_path, sep="\t", dtype=str)
        df_best = pd.read_csv(best_path, sep="\t", dtype=str)

        # Ensure sequence_id column exists
        if "sequence_id" not in df_pass.columns or "sequence_id" not in df_best.columns:
            raise ValueError("Both TSVs must contain a 'sequence_id' column.")

        # Find shared rows and columns
        shared_ids = set(df_pass["sequence_id"]) & set(df_best["sequence_id"])
        shared_cols = [c for c in df_pass.columns if c in df_best.columns and c != "sequence_id"]

        # Align on sequence_id
        df_best_indexed = df_best.set_index("sequence_id")

        # Overwrite pass values with best values for shared cols/ids
        for col in shared_cols:
            df_pass.loc[df_pass["sequence_id"].isin(shared_ids), col] = (
                df_pass.loc[df_pass["sequence_id"].isin(shared_ids), "sequence_id"]
                .map(df_best_indexed[col])
            )

        # Save
        df_pass.to_csv(out_path, sep="\t", index=False)

    @staticmethod
    def clean_scratch_dir(scratch_dir: str, output_dir: str) -> None:
        """
        Copy all .fa files from scratch_dir to output_dir (renaming to strip 'fastq' or 'fq'),
        then remove scratch_dir.

        Args:
            scratch_dir (str): Temporary working directory with intermediate files.
            output_dir (str): Permanent directory to keep the final .fa results.
        """
        os.makedirs(output_dir, exist_ok=True)

        for fname in os.listdir(scratch_dir):
            if fname.endswith(".fa"):
                # strip 'fastq' or 'fq' from the filename
                new_name = re.sub(r'(.fastq|.fq)', '', fname)
                src = os.path.join(scratch_dir, fname)
                dst = os.path.join(output_dir, new_name)

                shutil.copy2(src, dst)  # copy with metadata

        # Remove the scratch directory completely
