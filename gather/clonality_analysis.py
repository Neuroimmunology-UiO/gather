...
def main():
    ...
    parser.add_argument('--chain', required=True, choices=['heavy', 'light', 'mixed'], help='Type of BCR chain: heavy or light or mixed')
    parser.add_argument('--output_dir', default='.', help='Directory where the output files will be saved')
    parser.add_argument('--run_combined', action='store_true', help='Run the combined heavy and light chain analysis if both chains are present')
    parser.add_argument('--clonal_plot', action='store_true', help='Generate clonal plots if run_combined is successful')
    parser.add_argument('--plot_output_file', default='clonal_plot.png', help='Filename for the clonal plot')
    ...
    fasta_file = Path(args.fasta_file).expanduser()
    ...
    if args.chain == 'heavy':
        ...
        output_file = output_dir / "heavy_light_chains.fmt7"
    ...
    if args.chain == 'heavy':
        ...
        output_name = 'heavy_chains_db-pass.tsv'
    elif args.chain == 'light':
        output_name = 'light_chains_db-pass.tsv'
        ...
        output_name = 'heavy_light_chains_db-pass.tsv'
    ...
    # If the chain type is heavy, run DefineClones
    if args.chain == 'heavy':
        input_tsv = os.path.join(output_dir, "heavy_chains_db-pass.tsv")
    ...
    if args.run_combined:
        ...
        db_pass_tsv_heavy = os.path.join(output_dir, "heavy_chains_db-pass_clone-pass.tsv")
        db_pass_tsv_light = db_pass_tsv_heavy.replace('heavy', 'light').replace('db-pass_clone-pass', 'db-pass')

        if os.path.exists(db_pass_tsv_heavy) and os.path.exists(db_pass_tsv_light):
            ...
            df2 = pd.read_csv(db_pass_tsv_light, sep='\t')

            c_calls = df1['c_call']
            unique_c_calls = c_calls.unique()
            color_dict = {c_call: colors[i % len(colors)] for i, c_call in enumerate(unique_c_calls)}
            df1['color'] = df1['c_call'].map(color_dict)

            
            df1['cell_id'] = df1['sequence_id']
            df2['cell_id'] = df2['sequence_id']
            ...
            df1 = process_bcr_heavy_chains_cds(df1, database)
            ...
            if args.clonal_plot:
                def create_clone_plot(df_clone, color_dict, output_file):
                    ...
                    c_calls = df_clone['c_call'].tolist()
                    ...
                    nx.draw_networkx_nodes(G, pos, node_size=100, node_color=node_colors, alpha=0.7, edgecolors='black')
                    ...
                    plt.legend(handles=handles, loc='best', ncol=1, frameon=False, fontsize=20, title_fontsize=18)
