    ...
    if len(r1_fastq) == 1 and len(r2_fastq) == 1:
        try:
            ...
            if success:
                ...
                headers_contigs, sequences_contigs = [], []

                if os.path.exists(path_to_bcr):
                    headers_bcr, sequences_bcr = IO.read_fasta(path_to_bcr, imgt_fasta=False)

                if os.path.exists(path_to_bcr_contigs):
                    headers_contigs, sequences_contigs = IO.read_fasta(path_to_bcr_contigs, imgt_fasta=False)

                headers_unitigs, sequences_unitigs = IO.read_fasta(path_to_unitigs, imgt_fasta=False)

                # Combine the available sequences
                headers = headers_bcr + headers_contigs
                sequences = sequences_bcr + sequences_contigs
                ...
                if clean_cells_data:
                    if os.path.exists(path_to_bcr_contigs):
                        clean_directory(subdir, path_to_bcr_contigs)
                    else:
